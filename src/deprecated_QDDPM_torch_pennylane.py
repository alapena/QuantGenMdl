...
class DiffusionModel(nn.Module):
    ...
    def scrambleCircuit_t(self, t, input, phis, gs=None):
        ...
        def circuit():
            ...
            qml.StatePrep(input, wires=range(self.n))
            
            for tt in range(t):
                # Single qubit rotations
                for i in range(self.n):
                    qml.RZ(phis[3*self.n*tt + i], wires=i)
                    qml.RY(phis[3*self.n*tt + self.n + i], wires=i)
                    qml.RZ(phis[3*self.n*tt + 2*self.n + i], wires=i)
                
                # Homogeneous RZZ on every pair of qubits (n >= 2)
                if self.n >= 2:
                    for i, j in combinations(range(self.n), 2):
                        # Note: PennyLane's IsingZZ(phi) = RZZ(2*phi) in some other conventions
                        # Here we use the same scaling as in the original code
                        qml.IsingZZ(gs[tt]/(2*np.sqrt(self.n)), wires=[i, j])
...
def backCircuit(input, params, n_tot, L):
    ...
    L: layers of circuit
    '''
    dev = qml.device("default.qubit", wires=n_tot)

    @qml.qnode(dev)
    def circuit():
        for l in range(L):
            for i in range(n_tot):
                qml.RX(params[2*n_tot*l+i], wires=i)
                qml.RY(params[2*n_tot*l+n_tot+i], wires=i)
...
class QDDPM(nn.Module):
    ...
    def backwardOutput_t(self, inputs, params):
        ...
        output_full = torch.zeros_like(inputs)
        for i in range(inputs.shape[0]):
            output_full[i] = backCircuit(inputs[i], params, self.n_tot, self.L)
        # perform measurement
