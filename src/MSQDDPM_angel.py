...
class MSQDDPM(nn.Module):
    ...
    def _generate_haar_states(self, n_data):
        ...
        theta = torch.acos(2 * params[:, 2] - 1) # Sample from 1/2*sin distribution (not uniform). To ensure Haar.  

        def circuit_fn(phi, theta, omega):
            c = tc.Circuit(1)
            c.rz(0, theta=phi)
            c.ry(0, theta=theta)
            c.rz(0, theta=omega)
        ...
        vmap_circuit = K.vmap(circuit_fn, vectorized_argnums=(0, 1, 2))
        return vmap_circuit(phi, theta, omega)
